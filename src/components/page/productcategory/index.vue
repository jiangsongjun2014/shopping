		...
		<div class="main">
			<el-table ref="adminTable" :data="list" style="display: table-cell;" v-loading="listLoading" border >
				<el-table-column label="编号"  align="center">
					<template slot-scope="scope">{{scope.row.product_like_id}}</template>
				</el-table-column>
				...
				<!-- <el-table-column label="是否启动" align="center">
					<template slot-scope="scope">
					    ...
					    :active-value="1"
					    :inactive-value="0"
					    v-model="scope.row.product_like_status">
						</el-switch>
					</template>
				</el-table-column> -->
...
<script>
	...
	import {procategorylist, deleteprocategory, updateprocategorystatus} from "@/api/productcategory"
	...
	export default {
		...
		methods: {
			...
			getList(){
				...
				procategorylist(this.listQuery).then(res => {
			...
			handleDelete(index, row) {
				this.$confirm('是否要删除该类别?', '提示', {
					...
					deleteprocategory(row).then(response => {
			...
			handleHiddenChange(index, row) {
				var data = {product_like_status: row.product_like_status, product_like_id: row.product_like_id}
				console.log(data)
				...
				// 	type: 'warning'
				// }).then(() => {
					updateprocategorystatus(data).then(response => {
